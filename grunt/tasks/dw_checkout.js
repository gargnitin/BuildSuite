...
function gitClone(grunt, dependency, checkoutpath)
{
    ...
    if(dependency.repository.username || dependency.repository.password) {
        ...
        url = 'https://' + encodeURIComponent(dependency.repository.username) + ':' +  encodeURIComponent(dependency.repository.password) + '@' + url.substring(8);
    }
...
{
    grunt.registerMultiTask('dw_checkout', 'Prepare and execute VCS checkout tasks according to dependency setup.', function() {
        ...
        dependencies.forEach(function(dependency) {
            ...
            switch(dependency.repository.type.toLowerCase()) {
                ...
                default:
                    grunt.fail.warn('Unkown repository type for "' + dependency.id + '". Please configure.');
            }
        });
    });
};
