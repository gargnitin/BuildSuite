...
module.exports = function (grunt) 
{
    ...
    function deleteArchive(httpOptions, grunt, data, callback, retries) {
        ...
        var deleteRequest = http.request(httpOptions, function (res) {
            ...
            if (res.statusCode === 204) {
                ...
                grunt.log.error('Unknown error occurred (delete)!');
            }
            ...
            if(success) {
                ...
                //Retry a couple of times and then give up
                if (--retries) {
    ...
    {
        if(options.two_factor.enabled.toLowerCase() !== "true") {
             return;
        }
    ...
    {
        var user = grunt.config('environment.webdav.username');
        var pass = grunt.config('environment.webdav.password');
    ...
    function createUnzipTask(grunt, options, fileName)
    {
        var httpOptions = {
            auth: {
                user: grunt.config('environment.webdav.username'),
                pass: grunt.config('environment.webdav.password')
            },
