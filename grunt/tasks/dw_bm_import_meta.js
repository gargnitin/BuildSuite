...
module.exports = function(grunt) {
    grunt.registerMultiTask('dw_bm_import_meta', 'Import Meta data', function() {
        ...
        function selectValidationJob(body) {
            ...
            //Check if validation has been done on this file.
            var archiveLabel = bmUtils.removeAllWhiteSpaces('Meta Data Validation <{0}>'.replace('{0}', options.archiveName)),
                $td = $('form[name="ImpexForm"] > table:nth-child(6) > tr >td:nth-child(2)'),
                importLink;

            //compare with stripped whitespaces, demandware has random spacing in td element
            var record = $td.filter(function() {
                return bmUtils.removeAllWhiteSpaces($(this).text()) === archiveLabel;
            });
        ...
        request.post({
            url: options.server + '/on/demandware.store/Sites-Site/default/ViewCustomizationImpex-Start',
            jar: true
        }, function(error, resp, body) {
