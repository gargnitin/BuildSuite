module.exports = function(grunt) {
    grunt.registerMultiTask('dw_insert_buildinfo', 'Insert Build Information into Storefront Toolkit', function() {
        /**
        ...
        function insertInformation(template) {
            ...
            template = template.replace(/@DEPLOYMENT_PROJECT@/, grunt.config.get('settings.build\\.project\\.name') || '(not set)');
            template = template.replace(/@DEPLOYMENT_VERSION@/, grunt.config.get('settings.build\\.project\\.version') || '(not set)');
            template = template.replace(/@REVISION@/, grunt.config.get('settings.build\\.project\\.number') || '(not set)');
        ...
        if(!grunt.file.exists(cartridgePath)) {
            grunt.log.warn('Unable to insert Build info: Target cartridge ("' + options.cartridge + '") not included in this build.');
            return;
        }
        ...
        if(!grunt.file.exists(includeTemplateFile)) {
            grunt.log.warn('Unable to include Build info: Target template ("' + options.template + '")  does not exist.');
            return;
        }
