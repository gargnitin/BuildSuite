...
module.exports = function(grunt) {
     ...
     * @param {bool} useRealm - Store export globally on realm?
     *
     * @return {string}
     */
    ...
    var getConfigurationUnits = function(unitsToExport, sitesToExport) {
        ...
        if (!sitesToExport) {
            grunt.fail.warn('settings.site_export.sites should contain an array of sites to export in the main config.json');
        }
    ...
    grunt.registerMultiTask('dw_bm_export_units', 'Export data units', function() {
        ...
        var formUnits = getConfigurationUnits(this.data.unitsToExport, grunt.config('settings')['site_export.sites']);
