...
{
    grunt.registerMultiTask('dw_merge', 'Execute merge tastk for one repository.', function() {
        ...
        if (!options.optimize_css && !options.optimize_css) {
        ...
        this.files.forEach(function(filesEntry) {
            filesEntry.src.forEach(function(filePath) {
                  ...
                  if(options.optimize_js && content.indexOf('<!--- BEGIN JS files to merge') > 0) {
                      content = mergeJavascript(content, grunt, filePath);
                      foundOne = true
                  }
                  
                  if(options.optimize_css && content.indexOf('<!--- BEGIN CSS files to merge') > 0) {                     
...
function mergeJavascript(content, grunt, sourcePath) 
{
    ...
    content = content.replace(/<!--- BEGIN JS files to merge(.*)--->([\s\S]*)\s*<!--- END JS files to merge(.*)--->/g, function (all, params, scripts) {
        ...
        var sourceFiles = scripts.replace(/.*\('/g, cartridgePath + '/' + relpath ).replace(/'\).*/g, '').match(/[^\r\n]+/g).filter(function (e) {
            return e.length && e.indexOf('.js') > -1;
        });

        //Build full target file path + task name
        var targetFile = grunt.config('dw_properties').folders.code + cartridgeName + '/' + relpath + '/' + target;
        var targetTaskName = cartridgeName + '_' + target.split('.')[0].replace(/\/|\\/, '_');
