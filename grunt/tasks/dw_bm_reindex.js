...
function parseIndexList(grunt, body) {
    ...
    var result = $('input[name=ObjectUUID]').map(function() {
        return $(this).val();
    }).get();
...
function lookupIndex(grunt, done) {
    ...
    request({
        url: server + '/on/demandware.store/Sites-Site/default/ViewSearchIndexList_52-Start?SelectedMenuItem=search&CurrentMenuItemId=search',
        headers: {
            'Content-Type': 'text/html'
        },
        jar: true
...
function triggerReindex(grunt, indexes, done) {
    ...
    var formString, currentIndexUUID;
...
module.exports = function(grunt) {
    grunt.registerMultiTask('dw_bm_reindex', 'Business Manager Reindex of all sites', function() {
        var options = this.options(), done = this.async(), url = options.server + '/on/demandware.store/Sites-Site/default/ViewApplication-SelectSite';
